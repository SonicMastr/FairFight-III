module.exports = {
	...
	run: async (ff, m, args) => {
		...
		if(args[0]) {
			...
			embed.setAuthor(command.config.name, ff.user.avatarURL());
		...
		if (!args.length) {
			...
			embed.setAuthor('ff Screenshare Commands', ff.user.avatarURL());
			embed.setDescription(`\nYou can send \`${ff.prefix}help [command name]\` to get info on a specific command!`);
			...
			for (let i = 0; i < newCmds.length; i++) {
				...
				if (!thisCmd.config.disabled && !thisCmd.config.owner) {
					...
					else if (thisCmd.config.mod) {
						mod.push('``' + thisCmd.config.name + '``');
					}
					else {
			...
			if (mod.length) embed.addField('Moderation Commands', mod.join(' '));
